...
void USB_BattleWidget::NativeOnInitialized()
{
	...
	if (OwnerPC)
	{
		...
		OwnerPC->NewOwnedShipEvent.AddUniqueDynamic(this, &USB_BattleWidget::OnNewOwnedShip);
	}
}

void USB_BattleWidget::OnNewOwnedShip(ASB_Ship* const NewOwnedShip)
{
...
void USB_BattleWidget::OnShipDurabilityUpdated(float NewDurability)
{
	//UE_LOG(LogTemp, Display, TEXT("USB_BattleWidget::OnShipDurabilityUpdated %s"), FString::FromInt(NewDurability));
